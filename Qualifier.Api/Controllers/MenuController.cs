...
namespace Qualifier.Api.Controllers
{
    ...
    {
        [HttpGet("Role")]
        public async Task<IActionResult> GetAll(int roleId, [FromServices] IGetAllMenusByRoleIdQuery query)
        {
            ...
            var res = await query.Execute(roleId, companyId);
        ...
        [HttpGet()]
        public async Task<IActionResult> Get(int skip, int pageSize, string? search, [FromServices] IGetMenusByCompanyIdQuery query)
        {
            var accessToken = await HttpContext.GetTokenAsync("access_token");
            int companyId;
        ...
        [HttpGet("{id}")]
        public async Task<IActionResult> Get(int id, [FromServices] IGetMenuByIdQuery getMenuByIdQuery)
        {
            var res = await getMenuByIdQuery.Execute(id);
            if (res.GetType() == typeof(BaseErrorResponseDto))
        ...
        public async Task<IActionResult> Create([FromBody] CreateMenuDto model,
            [FromServices] ICreateMenuCommand createMenuCommand)
        {
            var accessToken = await HttpContext.GetTokenAsync("access_token");
            int userId;
            ...
            var res = await createMenuCommand.Execute(model);
            if (res.GetType() == typeof(BaseErrorResponseDto))
                return BadRequest(res);
            else
                return Ok(new
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put([FromBody] UpdateMenuDto model, int id,
            [FromServices] IUpdateMenuCommand updateMenuCommand)
        {
            var accessToken = await HttpContext.GetTokenAsync("access_token");
            int userId;
            ...
            var res = await updateMenuCommand.Execute(model, id);
            if (res.GetType() == typeof(BaseErrorResponseDto))
                return BadRequest(res);
            else
                return Ok(new
        ...
        [HttpDelete("{id}")]
        public async Task<IActionResult> delete(int id, [FromServices] IDeleteMenuCommand deleteMenuCommand)
        {
            var accessToken = await HttpContext.GetTokenAsync("access_token");
            int userIdValue;
            ...
            var res = await deleteMenuCommand.Execute(id, userId);
            if (res.GetType() == typeof(BaseErrorResponseDto))
                return BadRequest(res);
            else
                return Ok(new
