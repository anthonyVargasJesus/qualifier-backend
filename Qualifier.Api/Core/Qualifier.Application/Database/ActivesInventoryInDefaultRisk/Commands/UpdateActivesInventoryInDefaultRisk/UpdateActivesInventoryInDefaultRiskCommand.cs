    ...
    public class UpdateActivesInventoryInDefaultRiskCommand : IUpdateActivesInventoryInDefaultRiskCommand
    {
        ...
        public async Task<Object> Execute(UpdateActivesInventoryInDefaultRiskDto model, int id)
        {
            try
            {
                ...
                var updatedEntity = await _databaseService.ActivesInventoryInDefaultRisk
                             .AsNoTracking()
                             .FirstOrDefaultAsync(item =>
                                 item.activesInventoryInDefaultRiskId == id &&
                                 !(item.isDeleted ?? false));

                return _mapper.Map<UpdateActivesInventoryInDefaultRiskDto>(updatedEntity);
            }
            catch (Exception)
            {
