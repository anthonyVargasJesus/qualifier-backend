...
namespace Qualifier.Domain.Entities
{
    public class StandardEntity : BaseEntity
    {
        ...
        public void setRequirementsWithChildren(List<RequirementEntity> allRequirements)
        {
            // 1. Filtramos los nodos ra�z (Nivel 1)
            requirements = allRequirements
                .Where(x => x.level == 1)
                .OrderBy(x => x.numeration)
                .ToList();

            // 2. Procesamos cada ra�z de forma recursiva
            foreach (var req in requirements)
            {
                BuildTree(req, allRequirements);
            }

            // 3. Tu l�gica de numeraci�n final
            setNumeration(requirements);
        }
            ...
            string parentBreadcrumb = "")
        {
            ...
            {
                // Determinar numeraci�n del item actual
        ...
        private RequirementEvaluationEntity getDefaultRequierementeEvaluation(RequirementEntity requirement)
        {
            return new RequirementEvaluationEntity
            {
                ...
                percentage = 0,
                auditorStatus = 0,
                // Precarga el responsable por defecto de la cláusula (si se configuró uno) para
                // no obligar a elegirlo de nuevo en cada evaluación; sigue siendo editable y solo
                // se guarda de verdad cuando se hace el primer POST de esta evaluación.
